...
function App(props) {
  ...
  const stakingTimeLeft = useContractReader(readContracts, "Staker", "stakingTimeLeft");
  console.log("⏳ Staking Time Left:", stakingTimeLeft);

  const tempBalanceToWithdraw = useContractReader(readContracts, "Staker", "tempBalanceToWithdraw");
  console.log("💵💵💵 Temp Balance to Withdraw:", tempBalanceToWithdraw);

  const exponentialBalanceToWithdraw = useContractReader(readContracts, "Staker", "exponentialBalanceToWithdraw");
  console.log("💵💵💵 Exponential Balance to Withdraw:", exponentialBalanceToWithdraw);

  const [valueToStake, setValueToStake] = useState();
      ...
      <BrowserRouter>
        ...
        <Switch>
          <Route exact path="/">
            ...
            <div style={{ padding: 8, marginTop: 16, fontWeight: "bold"}}>
              <div>Staking Period Left:</div>
              {stakingTimeLeft && humanizeDuration(stakingTimeLeft.toNumber() * 1000)}
            </div>
            ...
            <input
              ...
              value={valueToStake}
              onChange={e => setValueToStake(e.target.value)}
            />
            </div>
            ...
            <div style={{ padding: 8 }}>
              <Button
                ...
                onClick={() => {
                  tx(writeContracts.Staker.stake({ value: ethers.utils.parseEther(valueToStake) }));
                }}
              >
                🥩 Stake ether!
              </Button>
            ...
            <div style={{ padding: 8 }}>
              <Button
                type={balanceStaked ? "success" : "primary"}
                onClick={() => {
                  tx(writeContracts.Staker.retrieveAndRedeposit());
                }}
              >
              🛎️ Retrieve back the locked up ether
              </Button>
            </div>
