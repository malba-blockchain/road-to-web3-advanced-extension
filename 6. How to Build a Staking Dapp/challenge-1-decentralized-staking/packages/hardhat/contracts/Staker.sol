...
contract Staker is Ownable  {
  ...
  uint256 public stakingDeadline = block.timestamp + 45 seconds; //Staking in the smart contract is only available for the first 2 min
  uint256 public claimDeadline = block.timestamp + 75 seconds; //Claim deadline is only available until 4 min
  ...
  modifier stakingDeadlineReached(bool requireReached){
    uint256 timeRemaining = stakingTimeLeft();

    if(requireReached) {
      require(timeRemaining ==0, "End of staking period is not reached yet");
    }
    else {
      require(timeRemaining >0, "Staking period has been reached");
    }
    _;
  }
  ...
  function stake() public payable stakingDeadlineReached(false) claimDeadlineReached(false) {
  ...
  // Stake function for a user to stake ETH in our contract
  function retrieveAndRedeposit() public payable onlyOwner {
    
    exampleExternalContract.retrieveLockedUpStake();
    emit Stake(address(exampleExternalContract), address(exampleExternalContract).balance);
  }

  /*
    Allows any user to repatriate "unproductive" funds that are left in the staking contract
    past the defined withdrawal/claimable period and send it to the external contract
  */
  ...
  function stakingTimeLeft() public view returns(uint256 staking_Time_Left) {
