...
contract Casino {
  ...
  //Side A has 1 day to show back the value of its random number, otherwise he automatically loses the money
  uint forfeitPeriod = 20 seconds;
  ...
  // Called by sideB in case sideA didn't reavealed the random value in a 24 hour period
  function executeForfeit (uint _commitment) external {

      require(proposedBet[_commitment].accepted, "Bet has not even been accepted yet");

      require(acceptedBet[_commitment].sideB == msg.sender, "Not a bet you accepted as sideB, or wrong value");

      require(block.timestamp >= acceptedBet[_commitment].acceptedAt + forfeitPeriod, "The forfeit period has not elapsed yet");

      //Send the funds to sideB, because sideA didnt reveal the random value on time
