...
//COLLECTION ADDRESS ON ETHERSCAN 0xBcC8cf22f25EED48B67b63bF5a566a7fA10A7A32
...
contract NFTMarketplace is ERC721URIStorage {
    ...
    struct ListedToken {
         ...
         address payable originalNFTCreator;
         uint256 price;
    ...
    function executeSale(uint256 _tokenId) public payable {
        ...
        address tokenOwner = idToListedToken[_tokenId].tokenOwner;


        //Split the Sale into 2 cases. 
        //Case A: Its the first sale, the smart contract is the owner
        //Case B: Its not the first sale, a user account is the owner

        //CODE FOR THE CASE A
        if(tokenOwner == address(this)){

            /*Code related with the transaction of value*/
            ...
            uint256 paymentValue = (msg.value*90)/100;
            payable(seller).transfer(paymentValue);

            // Transfer the royalties to the original creator of the NFT 
            address originalNFTCreator = idToListedToken[_tokenId].originalNFTCreator;

            uint256 royaltiesValue = (msg.value*10)/100;
            payable(originalNFTCreator).transfer(royaltiesValue);



            /*Code related with the transaction of the NFT*/
            ...
            console.log("Token Owner value before transfer");
            console.log(tokenOwner);

            console.log("Address of the transaction sender");
            ...
            //Update who is the new token owner
            idToListedToken[_tokenId].tokenOwner = payable(msg.sender);

            console.log("Token Owner value after transfer");
            console.log(idToListedToken[_tokenId].tokenOwner );


        }
        else { //CODE FOR THE CASE B
